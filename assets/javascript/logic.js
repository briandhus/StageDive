...
$(document).ready(function() {
				...
				if (events.length === 0) {
					...
					$.each(events, function(index, event) {
						...
						if (locationInput) {
							...
							if (sampleRecord.eventTicketURL == "") {
	...
	var cheapestAirFare = function(sampleRecord) {
		if (sampleRecord.userAirport === sampleRecord.eventAirport) {
			...
			for (var i = 0; i < allPossibleDates.length; i++) {
				console.log(i);
				var possibleDate = allPossibleDates[i];
				...
				$.ajax({
					...
					response.results.forEach(function(itinerary) {
						...
						if (i === allPossibleDates.length - 1) {
							updateScreen(sampleRecord);
						};
					});
				});
			};
	...
	var updateScreen = function(sampleRecord) {
